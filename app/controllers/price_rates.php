...
class price_ratesController extends coreController {
    public $model_name = 'store_price_rates';
...
//            $results = ;
    ...
    public function get_scope_values(){
        $this->mapRoute("scope");
        $this->checkRequired(["scope"], $this->parts);
        $rules = [
            "scope" => FILTER_SANITIZE_NUMBER_INT
        ];
        $validated = $this->sanitize($this->parts, $rules);

        switch ($validated['scope']) {
            case 1:
                $query = "select id, name from site_store_categories_tbl 
                inner join site_store_categories_lang_tbl on site_store_categories_tbl.id = site_store_categories_lang_tbl.article_id 
                where language_id = ".$this->langid." and active=1 order by id";
                break;
            case 2:
                $query = "select brand as id, brand as `name` from site_store_products_tbl  
                where active=1 and quantity>0 group by brand";
                break;
            case 3:
                $query = "select id, concat('[',sku,'] ',title) as `name` from site_store_products_tbl 
                inner join site_store_products_lang_tbl on site_store_products_tbl.id = site_store_products_lang_tbl.article_id 
                where language_id = ".$this->langid." and active=1 and quantity>0 order by title";
                break;
        }
