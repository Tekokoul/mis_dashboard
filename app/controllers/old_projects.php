...
//        debug($_SESSION);
        $query = "select * from pm_members_tbl where account=".$_SESSION['user']['user_id'];
//        debug($query);
...
//            $results = ;
    ...
    public function edit(){
        ...
        $this->AddJS("https://cdnjs.cloudflare.com/ajax/libs/blueimp-md5/2.18.0/js/md5.min.js");
        $this->AddJS("/js/pm_projects.js");
        $this->prepare_edit_mode();
        $this->render($data);
    }
    ...
    public function get_details(){
        ...
        if($validated['project_id']!=0) {
            $result = $this->DB->MQ("select * from ". $this->model->get_table_name($validated['model'])." where project_id=". $validated['project_id'], "all");
            $data['data'] = (count($result)>1) ? $result : $result[0];
        }
        $this->prepare_edit_mode();
...
//            $data = array_merge($data, $this->model->get_list_data($model, $page, $items_per_page, $validated['search-term']??""));
            $count_query = "select count(*) as total from " . $this->model->get_table_name($model['model_name'])." where JSON_CONTAINS(applies_to, '\"".$member."\"')";
            $data['count'] = $this->DB->MQ($count_query, "one")['total'];
            $query = "select * from ".$this->model->get_table_name($model['model_name'])." where JSON_CONTAINS(applies_to, '\"".$member."\"') limit " . $items_per_page . " offset " . (($page - 1) * $items_per_page);
            ...
            "common" => [
                ...
                "type" => [
                    ...
                    "values_list" => [
                        ...
                        "pm_projects_tasks" => "Multiple KPIs",
                        "pm_projects_percentages" => "Percentage",
                        "pm_projects_dates" => "Dates"
                    ],
                    "disabled" => true,
                    "no_update" => true
                ]
...
//        exit();
    ...
    public function get_tasks_details(){
        ...
        $data['fields'] = $this->model->get_list_fields($model);
        if($validated['project_id']!=0) {
            $result = $this->DB->MQ("select * from ". $this->model->get_table_name('pm_projects_tasks')." where project_id=". $validated['project_id'], "one");
            $data['data'] = json_from_db($result['tasks']);
            foreach ($data['data'] as $key=>$value){
                $data['data'][$key]['task_id'] = $key+1;
    ...
    public function get_task_details(){
        ...
        $tasks = json_from_db($result['tasks']);
        $data['data'] = [
            "task" => $tasks[$validated['id']-1]['name'],
    ...
    public function task_update(){
        ...
        if(is_set($result)){
            $query = "UPDATE `pm_progress_tasks_tbl` SET `result` = ".db_esc($validated['result']).", `progress_date` = '".$validated['progress_date']."', `comment` = '".db_esc($validated['comment'])."' 
            ...
            $query = "INSERT INTO `pm_progress_tasks_tbl` ( `member_id`, `project_id`, `result`, `task_id`, `progress_date`, `comment`) VALUES 
            ( ".$validated['member_id'].", ".$validated['project_id'].", ".db_esc($validated['result']).", ".$validated['task_id'].", '".$validated['progress_date']."', '".db_esc($validated['comment'])."' )";
            $executed = $this->DB->MQ($query);
        }
