...
<header class="page-header page-header-left-inline-breadcrumb">
    <h2 class="font-weight-bold text-6">Edit: <?= display($data['model_name']); ?></h2>
</header>
<form class="ecommerce-form action-buttons-fixed" action="<?=$this->L("remote/json_update")?>" method="post">
    <input type="hidden" name="tablename" value="<?= display($data['model_name']); ?>" >
    <input type="hidden" name="remote_host" value="<?= display($data['remote_host']); ?>" >
<div class="row">
    <?php
                ...
                <div class="row">
                    <div>
                        ...
                        foreach ($data['model']['common'] as $field=>$value) {
                            $prepopulated = (isset($data['data'][$field])) ? $data['data'][$field] : "";
                        ...
                        <div class="row">
                            <div>
                                ...
                                foreach ($properties as $field=>$value) {
                                    $prepopulated = (isset($data['data']['languages'][$language][$field])) ? $data['data']['languages'][$language][$field] : "";
